...
export const StateContextProvider = ({ children }) => {
  const { contract } = useContract(
    "0xC4d42d1b84437cCD82fF8331b20A5dAf1C05DD3d",
    TipperDapp.abi
  );
  const { mutateAsync: createElection } = useContractWrite(
    contract,
    "createElection"
  );
  const { mutateAsync: registerTipper } = useContractWrite(
    contract,
    "registerTipper"
  );
  const { mutateAsync: registerArtist } = useContractWrite(
    contract,
    "registerArtist"
  );
  ...
  const publishElection = async (
    ...
    try {
      const data = await createElection({
        args: [address, _electionName, _voteToken, _image, _candidateAddress],
      });
  ...
  const getElections = async () => {
    const elections = await contract.call("getElections");

    const parsedElections = elections.map((election, i) => ({
      owner: election.owner,
      electionName: election.electionName,
      voteToken: ethers.utils.formatEther(election.voteToken.toString()),
  ...
  const getUserElections = async () => {
    ...
    const filteredElections = allElections.filter(
      (election) => election.owner === address
    );
  ...
  const makeVote = async (_electionId, _candidateAddress, voteToken) => {
    const data = await contract.call(
      "makeVote",
      [_electionId, _candidateAddress],
      {
        value: ethers.utils.parseEther(voteToken),
      }
    );
  ...
  const publishTip = async (_artistAddress, token) => {
    try {
      const data = await contract.call("make_Tip", [_artistAddress], {
        value: ethers.utils.parseEther(token),
      });
      console.log(data);
      return data;
    } catch (error) {
